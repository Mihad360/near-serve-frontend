...
import { mockConversations } from "@/data/customerMock";
  ...
  return (
    <div>
      <PageHeader
        eyebrow="Inbox"
        title="Messages"
        description="Chat with providers about your jobs."
      />
      ...
      {sorted.length === 0 ? (
        <EmptyState
          ...
          description="When you book a provider, your chats will show up here."
        />
      ) : (
        <ul className="space-y-2.5">
          {sorted.map((conv, i) => (
            <li key={conv.id} className="animate-fade-up" style={{ animationDelay: `${i * 80}ms` }}>
              <Link
                href={ROUTES.CUSTOMER_CONVERSATION(conv.id)}
                className={cn(
